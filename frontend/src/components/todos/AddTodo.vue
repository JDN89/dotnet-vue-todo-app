...
<template>
    <div class="flex bg-gray-800 bg-opacity-50 fixed left-0 right-0 bottom-0 top-0 items-center">
        <div class="msg content-center min-h-44 max-h-lg w-full m-1 p-2 sm:max-w-70 sm:mx-auto mx-auto">
            <button :title="t('button.close')" class="float-right hover mt-1 mr-1"
            ...
            <div class="todos">
                <ul>
                    <div class="relative">
                        <li v-for="todo in todoStore.getTemporaryList!.todos" :key="todo"
                            class="flex justify-left items-start">
                            <carbon-checkbox @click="archiveTask(todoStore.getTemporaryList!.listId, todo)"
                                class="mr-2 my-auto hover" />{{ todo }}
                        </li>
                    </div>
                </ul>
            </div>
            <div class="flex justify-left items-start">
                ...
                <input v-else type="text" ref="newTodoRef" key="user-input" v-model="newTodo"
...
" />
            ...
            <div>
                <ul>
                    <li v-for="archived in todoStore.getTemporaryList!.archived" :key="archived"
                        class="flex justify-left items-start">
                        <carbon-checkbox-checked @click="unArchiveTask(todoStore.getTemporaryList!.listId, archived)"
